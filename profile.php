...
<?php
	...
	if(isset($_GET['search_btn'])){
		...
		$select = mysqli_query($connection, "SELECT * from questions inner join users on users.username = questions.username where questions.username = '$username' and title like '%$search%' order by post_id desc") or die(mysqli_error($connection));
	}

	else{
		$select = mysqli_query($connection, "SELECT * from questions inner join users on users.username = questions.username where questions.username = '$username' order by post_id desc") or die(mysqli_error($connection));
	}
?>
	...
	<div class="col-sm-12" id="post_container">
		<div class="col-md-1" id="like_dislike">
			<div class="col-sm-12" id="like" >
				<span>250</span><p>&nbsp; <i class="fa fa-thumbs-up"></i></p>
			</div>
			<div class="col-md-12" id="like">
				<span>300</span><p>&nbsp; <i class="fa fa-thumbs-down"></i></p>
			</div>
		</div>
		<div class="col-md-11" id="post_question">
			...
			<div class="col-sm-12" id="post_comment">
				...
				<div class="col-sm-4" id="comment">
					<a href="">Like</a>&nbsp;&nbsp; <a href="">Dislike</a>&nbsp;&nbsp; <?php echo "<a href=comment.php?id=".$row ['post_id'].">Comment</a> "; ?>&nbsp;&nbsp;<?php 
...
<?php
	...
	if(isset($_POST['add_post'])){
		...
		$insert = mysqli_query($connection, "INSERT into questions(post_id, title, details, date, username) values('$post_id', '$title', '$details', '$date', '$username')") or die(mysqli_error($connection));
			if($insert){
